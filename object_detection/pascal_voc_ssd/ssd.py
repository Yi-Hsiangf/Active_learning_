...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes):
        ...
        if phase == 'test':
            ...
            self.detect = Detect(num_classes, 0, 200, 0.01, 0.45, False)
        if phase == 'test_al':
            self.softmax = nn.Softmax(dim=-1)
            self.detect = Detect(num_classes, 0, 200, 0.01, 0.45, True)
    ...
    def forward(self, x):
        ...
        """
        sources = list()
        self.active_learning_features = list()
        loc = list()
        conf = list()
        ...
        elif self.phase == "test_al":
            #print("test al ssd")
            conf_preds = self.softmax(conf.view(conf.size(0), -1,
                                       self.num_classes))
            output, output_idx_mem = self.detect.forward(
                self.detect,
                loc.view(loc.size(0), -1, 4),  # loc preds
                conf_preds,  # conf preds
                self.priors.type(type(x.data))  # default boxes
            )
            return output, output_idx_mem, conf_preds
        else: 
            #print("train ssd")
            output = (
                loc.view(loc.size(0), -1,
...
def build_ssd(phase, size=300, num_classes=21):
    if phase != "test" and phase != "train" and phase != "test_al":
        print("ERROR: Phase: " + phase + " not recognized")
        return
    if size != 300:
        print(
            "ERROR: You specified size " + repr(size) + ". However, " +
            "currently only SSD300 (size=300) is supported!"
        )
        return
    base_, extras_, head_ = multibox(
        vgg(base[str(size)], 3), add_extras(extras[str(size)], 1024),
