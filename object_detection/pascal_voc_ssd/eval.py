...
imgsetpath = os.path.join(
    args.voc_root, 'VOC2007', 'ImageSets', 'Main', '{:s}.txt'
)
YEAR = '2007'
...
def test_net(
    ...
    #    (x1, y1, x2, y2, score)
    all_boxes = [
        [[] for _ in range(num_images)] for _ in range(len(labelmap) + 1)
    ]
    ...
    for i in range(num_images):
        ...
        for j in range(1, detections.size(1)):
            ...
            #print("mm: ", dets[:, 0].gt(0.))
            #print("expand: ", dets[:, 0].gt(0.).expand(5, dets.size(0)))
...
if __name__ == '__main__':
    ...
    for cycle in range(10):
        weight_path = 'weights/Active_learning/lr_0.0001_cycle_'+str(cycle+1)+'k.pth'
        net = build_ssd('test', 300, num_classes)  # initialize SSD
        net.load_weights(weight_path)
        net.eval()
